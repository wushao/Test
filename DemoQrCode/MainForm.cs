            ...
            foreach (DataGridViewRow row in this.dgBooks.Rows)
            {
                if (row.Selected)
                {
                   Book book = row.DataBoundItem as Book;
                    ...
                    this.pbIsbn.Image = BarcodeHelper.Generate2(book.ISBN, this.pbIsbn.Width, this.pbIsbn.Height);
                    this.pbUrl.Image = BarcodeHelper.Generate1(book.Url,this.pbUrl.Width,this.pbUrl.Height);
                    this.pbUrl2.Image = BarcodeHelper.Generate3(book.Url, this.pbUrl2.Width, this.pbUrl2.Height);
                }
            }
        }
    }
